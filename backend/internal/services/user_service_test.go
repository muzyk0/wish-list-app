...
// makeDBUser constructs a db.User with the given fields for test fixtures.
func makeDBUser(id pgtype.UUID, email, passwordHash, firstName, lastName, avatarURL string) db.User {
	return db.User{
		ID:           id,
		Email:        email,
	...
	t.Run("empty optional fields produce invalid pgtype.Text", func(t *testing.T) {
		...
		mockRepo := &UserRepositoryInterfaceMock{
			...
			CreateFunc: func(ctx context.Context, user db.User) (*db.User, error) {
				...
				created := db.User{ID: createdID, Email: user.Email, PasswordHash: user.PasswordHash}
				return &created, nil
			},
		}
		svc := NewUserService(mockRepo)
...
func TestUserService_ChangeEmail(t *testing.T) {
	...
	t.Run("allows changing to same email (own account)", func(t *testing.T) {
		...
		mockRepo := &UserRepositoryInterfaceMock{
			...
			UpdateFunc: func(ctx context.Context, u db.User) (*db.User, error) {
				return &u, nil
			},
		}
		svc := NewUserService(mockRepo)
...
func TestUserService_DeleteUser(t *testing.T) {
	...
	t.Run("propagates repo error", func(t *testing.T) {
		repoErr := fmt.Errorf("failed to delete user: %w", repositories.ErrUserNotFound)
		mockRepo := &UserRepositoryInterfaceMock{
			DeleteFunc: func(ctx context.Context, id pgtype.UUID) error {
				return repoErr
			},
