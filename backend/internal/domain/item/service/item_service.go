...
func (s *ItemService) GetMyItems(ctx context.Context, userID string, filters repository.ItemFilters) (*PaginatedItemsOutput, error) {
	// Parse user ID
	ownerID := pgtype.UUID{}
...
// Helper function to convert models.GiftItem to ItemOutput
func (s *ItemService) convertToOutput(item *models.GiftItem) *ItemOutput {
	output := &ItemOutput{
		ID:          item.ID.String(),
